...
class AC(nn.Module):
    ...
    def __init__(self):
        ...
        self.alpha_head = nn.Sequential(nn.Linear(128, 1), nn.Softplus())
        self.beta_head = nn.Sequential(nn.Linear(128, 1), nn.Softplus())
...
for s in range(10000):
    ...
    for _ in range(2000):
        ...
        status,reward,done,_=env.step(action.cpu().numpy()*4-2)
    ...
    if s%10==0:    
        print('step :{} score:{}'.format(s,rewards_count/2000))

    
    writer.add_scalar('running_score',rewards_count/2000,s)
    ...
    for _ in range(ppo_epoch):
        ...
        (alpha ,beta),nvalues=model(status_set)
