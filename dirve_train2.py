...
def random_shadow(image):
    ...
    image[y:y+height, x:x+width, 2] = image[y:y +
                                            height, x:x+width, 2]*bright_factor
...
record = []
for doc in docs:
    record.append(pd.read_csv(doc, names=[
...
class my_dataset(Dataset):
    ...
    def __getitem__(self, idx):
      ...
      #  pic_root='/home/xu/Downloads/'+pic_root[8:].replace('\\','/')
        ...
        pic = random_shadow(frame)
        pic=random_brightness(pic)
...
test_dataloader = DataLoader(
    valset, batch_size=batch_size, shuffle=True, num_workers=4)
