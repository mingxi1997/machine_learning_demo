...
def strategy_nn(status,e):
    ...
    a=torch.distributions.Bernoulli(e)
    if a.sample()==1:
...
def show_count():
    ...
    while not done:
        ...
        if count==2000:
            torch.save(model.state_dict(), 'pole_q.pth')  
...
# rewards=rewards-sum(rewards)/len(rewards)
    ...
    statuses=torch.tensor(status_set).to(torch.float32).to(device)
  ...
  for i in range(len(statuses)):
        optimizer.zero_grad()
        Q=model(statuses[i])
        q=Q[int(actions[i].item())]
        loss=criterion(q,rewards[i])
        loss.backward()
        mloss.add(loss.item())
        optimizer.step()
  mloss.show()
