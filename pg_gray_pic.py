...
def stack_work(series):
    ...
    for j in range(len(series)):
       ...
       if j>4:
           s.append(torch.cat((series[j].squeeze(0),series[j-1].squeeze(0),series[j-2].squeeze(0),series[j-3].squeeze(0))).unsqueeze(0))
 
           
       else:
           s.append(torch.cat((series[j].squeeze(0),series[j].squeeze(0),series[j].squeeze(0),series[j].squeeze(0))).unsqueeze(0))
    return s
...
def collect_data(times):
  ...
  for i in range(times):
    ...
    while not done:
        ...
        count+=1
        if count<4:
            action=strategy_raw(torch.cat((get_screen().squeeze(0),get_screen().squeeze(0),get_screen().squeeze(0),get_screen().squeeze(0))).unsqueeze(0))
        else:
            action=strategy_raw(torch.cat((status_set[-1].squeeze(0),status_set[-2].squeeze(0),status_set[-3].squeeze(0),status_set[-4].squeeze(0))).unsqueeze(0))
